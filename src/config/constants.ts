...
const PANTHEON_NAV: { slug: string; name: string; location: string }[] = [
  {
    slug: 'calus-resplendent',
    name: 'Calus, Resplendent',
    location: 'Pantheon',
  },
  {
    slug: 'morgeth-surpassing',
    name: 'Morgeth, Surpassing',
    location: 'Pantheon',
  },
  {
    slug: 'insurrection-prime-revolution',
    name: 'Insurrection Prime, Revolution',
    location: 'Pantheon',
  },
]
...
export const DESTINY_ACTIVITIES: Record<string, ActivityCategory> = {
  raids: {
    ...
    href: '/raids',
    description:
      'Hướng dẫn chi tiết từng bước cho các hoạt động endgame 6 người.',
    icon: Swords,
    items: RAID_NAV.map((r) => ({
      title: r.name,
      href: `/raids/${r.slug}`,
  ...
  pantheon: {
    ...
    href: '/pantheon',
    description:
      'Trải nghiệm đánh boss liên hoàn đỉnh cao. Đối đầu với những kẻ thù khó nhằn nhất Destiny 2.',
    icon: Crown,
    items: PANTHEON_NAV.map((p) => ({
      title: p.name,
      href: `/pantheon/${p.slug}`,
  ...
  exotic_missions: {
    ...
    description:
      'Nhiệm vụ đặc biệt ẩn chứa những món vũ khí Exotic uy lực nhất.',
    icon: Target,
    items: EXOTIC_MISSION_NAV.map((e) => ({
      title: e.name,
      href: `/exotic-missions/${e.slug}`,
  ...
  database: {
    ...
    href: '/database',
    description:
      'Cơ sở dữ liệu chọn lọc các trang bị META và mạnh nhất Destiny 2.',
    icon: Database,
    items: [
      {
        title: 'Exotic Armor',
        href: '/database/exotic-armor',
        description:
          'Tra cứu hệ thống giáp Exotic của 3 class và Exotic Class Items.',
      },
      {
        title: 'Exotic Weapons',
        href: '/database/exotic-weapons',
        description: 'Tra cứu thông tin và cách lấy các loại vũ khí Exotic.',
      },
      {
        title: 'Armor Sets',
        href: '/database/armor-sets',
        description:
