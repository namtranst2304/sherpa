...
- **Cách sử dụng:** Import các components đã cấu hình sẵn `variant` (màu sắc: `cyan`, `orange`, `zinc`, v.v.):
  ```tsx
  import { CyberCard, CyberHeading, CyberButton, CyberBadge } from "@/components/common/CyberComponents";

  // Thẻ chứa Neon
  <CyberCard variant="cyan" pulse={false} withCorners>
     <CyberHeading variant="default" size="lg">Tiêu đề chói lóa</CyberHeading>
     <CyberBadge variant="orange" pulse>NEW</CyberBadge>
